    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gradient-to-br from-purple-950/95 to-purple-900/95 backdrop-blur-sm rounded-2xl border border-amber-400/30 max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-amber-400/20">
          ...
          <button
            onClick={onClose}
            className="text-purple-300 hover:text-amber-400 transition-colors"
          >
        ...
        <div className="p-6">
          {isLoading ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-3 text-amber-400">Loading profile...</span>
            </div>
          ) : error ? (
            <div className="text-center py-8">
              <div className="text-red-400 text-lg mb-2">Error loading profile</div>
              <div className="text-purple-300 text-sm mb-4">{error}</div>
              <button 
                onClick={fetchProfile}
                className="px-4 py-2 bg-amber-500 text-purple-900 rounded-lg hover:bg-amber-400 transition-colors"
              >
                Try Again
              </button>
            </div>
            ...
            <div className="space-y-6">
              ...
              {/* Level and XP */}
              <div className="bg-purple-800/30 rounded-xl p-4 border border-amber-400/20">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-amber-400 font-bold">Level {profile.stats.level}</span>
                  <span className="text-purple-300 text-sm">{profile.stats.xp} XP</span>
                </div>
                <div className="bg-purple-900/60 rounded-full h-3 overflow-hidden border border-amber-400/40">
                  <div 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-purple-800/30 rounded-xl p-4 border border-amber-400/20 text-center">
                ...
                <div className="bg-purple-800/30 rounded-xl p-4 border border-amber-400/20 text-center">
                  ...
                  <div className="text-purple-300 text-sm">MVPs</div>
                </div>
                
                <div className="bg-purple-800/30 rounded-xl p-4 border border-amber-400/20 text-center">
                  <div className="text-2xl font-bold text-orange-400">{profile.stats.tournamentsWon}</div>
                  <div className="text-purple-300 text-sm">Tournaments Won</div>
                </div>
              </div>
              ...
              <div className="bg-purple-800/30 rounded-xl p-4 border border-amber-400/20">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-purple-300 text-sm">{(profile.stats.winRate * 100).toFixed(1)}%</span>
                </div>
                <div className="bg-purple-900/60 rounded-full h-3 overflow-hidden border border-amber-400/40">
                  <div 
                ...
                <div className="flex gap-3 justify-center">
                  {profile.stats.mvps > 0 && (
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-xs text-purple-300">{profile.stats.mvps} MVP{profile.stats.mvps > 1 ? 's' : ''}</span>
                    </div>
                  )}
                  ...
                  {profile.stats.tournamentsWon > 0 && (
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-xs text-purple-300">{profile.stats.tournamentsWon} Tournament{profile.stats.tournamentsWon > 1 ? 's' : ''}</span>
                    </div>
                  )}
                  ...
                  {profile.stats.wins > 0 && (
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-xs text-purple-300">{profile.stats.wins} Win{profile.stats.wins > 1 ? 's' : ''}</span>
                    </div>
                  )}
